...
function image_upload($img){
    ...
    {
       header("location: manage_products.php?alert=img_upload");
       exit;
...
function image_remove($img){
    if(!unlink(UPLOAD_SRC.$img)){
        header("location: manage_products.php?alert=img_rem_fail");
        exit;
...
if(isset($_POST['addproduct']))
{
   ...
   if( mysqli_query($con,$query)){
    ...
    header("location: manage_products.php?alert=add_failed");
   }
}
...
if(isset($_GET['rem']) && $_GET['rem']>0)
{
    ...
    if(mysqli_query($con,$query)){
        header("location: manage_products.php?success=removed");
    }
    else{
        header("location: manage_products.php?alert=remove_failed");
    }
}
   ...
   `description`='$_POST[desc]',`image`='$imgpath' WHERE `id`='$_POST[editpid]'";
    ...
    else
    {
        ...
        if(mysqli_query($con,$update)){
         ...
         {
            header("location: manage_products.php?alert=update_failed");
         }
